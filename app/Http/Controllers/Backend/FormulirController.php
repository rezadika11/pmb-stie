...
class FormulirController extends Controller
{
    ...
    public function index()
    {
        ...
        // Ambil data Tahun Akademik dengan status aktif
        $data['ThnAkademik'] = DB::table('tahun_akademik')
            ->leftJoin('mahasiswa', 'tahun_akademik.id', 'mahasiswa.id_tahun_akademik')
            ->where('tahun_akademik.status', 1)
            ->select('tahun_akademik.*') // Pastikan `tanggal_mulai` dan `tanggal_selesai` diambil
            ->first();

        // Tambahkan validasi apakah pendaftaran dibuka
        if ($data['ThnAkademik']) {
            // Ambil tanggal mulai dan selesai
            $tanggalMulai = Carbon::parse($data['ThnAkademik']->tanggal_mulai)->startOfDay(); // Mulai dari awal hari
            $tanggalSelesai = Carbon::parse($data['ThnAkademik']->tanggal_selesai)->endOfDay(); // Sampai akhir hari

            // Cek apakah sekarang berada di antara tanggal mulai dan selesai
            $data['ThnAkademik']->is_open = $now->between($tanggalMulai, $tanggalSelesai);
        }
