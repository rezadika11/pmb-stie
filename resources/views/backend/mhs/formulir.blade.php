                    ...
                    <div class="card-body">
                        @if (optional($mhs)->status_step == 5)
                            <table class="table table-striped text-dark">
                                ...
                                @if($mhs->status_pembayaran == 0 && $mhs->status_daftar == 0)
                                   <p class="text-primary">Silahkan upload bukti pembayaran</p>
                            ...
                            @elseif($mhs->status_pembayaran == 0 && $mhs->status_daftar == 2)
                            <p class="text-danger">Mohon maaf anda tidak diterima</p>
                            @elseif($mhs->status_pembayaran == 1 && $mhs->status_daftar == 1)
                                <!--<p class="text-primary">Selamat anda diterima</p>-->
                            ...
                            @endif
                        @else
                            @if ($ThnAkademik && $ThnAkademik->is_open)
                            ...
                            @else
                                  <!-- Pesan ketika pendaftaran belum dibuka -->
                                <div class="alert alert-danger mt-4">
                                    <h4>Pendaftaran belum dibuka</h4>
                                    <p>Silakan cek kembali di tanggal yang telah ditentukan.</p>
                                </div>
                            @endif
                        @endif
                    </div>
...
{{-- Fungsi kelas prodi --}}
<script>
    // Fungsi untuk mengatur opsi jenis kelas berdasarkan jenis pendaftaran
    function updateJenisKelas() {
        const jenisPendaftaran = $('#jenis_pendaftaran').val();
        const $jenisKelas = $('#jenis_kelas');

        // Reset pilihan jenis kelas
        $jenisKelas.find('option:not([value=""])').remove();

        // Logika untuk mengisi opsi jenis kelas
        if (jenisPendaftaran === 'kip') {
            // Jika KIP, hanya tampilkan kelas pagi
            $jenisKelas.append('<option value="pagi">Kelas Pagi</option>');
        } else {
            // Jika Reguler, tampilkan semua kelas
            $jenisKelas.append(
    ...
    // Event listener untuk perubahan jenis pendaftaran
